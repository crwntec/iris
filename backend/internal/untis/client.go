...
func ClientFromSession(config Config, token, sessionID, schoolID, username, password string) (*Client, error) {
	jar, err := cookiejar.New(nil)
	if err != nil {
		return nil, err
	}
	u, err := url.Parse(config.BaseURL + "/WebUntis")
	if err != nil {
		return nil, err
	}
	jar.SetCookies(u, []*http.Cookie{
		{Name: "JSESSIONID", Value: sessionID},
		{Name: "schoolname", Value: schoolID},
	})
	c := &Client{
		http: &http.Client{
			Jar:     jar,
...
func (c *Client) GetAbsences(ctx context.Context, info UntisInfo, start, end string) (Absences, error) {
	req, err := c.newAuthedRequest(ctx, "GET", c.config.BaseURL+"/WebUntis/api/classreg/absences/students")
	if err != nil {
		return Absences{}, fmt.Errorf("creating request failed: %w", err)
	}

	q := req.URL.Query()
	q.Add("startDate", start)
	q.Add("endDate", end)
	q.Add("studentId", fmt.Sprintf("%d", info.UserID))
